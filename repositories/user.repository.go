...
func (repository userRepository) GetAll() ([]models.User, error) {
	...
	if err := repository.DB.Preload("Tasks").Find(&users).Error; err != nil {
		return nil, err
	}
	return users, nil
}
...
func (repository userRepository) GetByID(id uint) (*models.User, error) {
	...
	if err := repository.DB.Preload("Tasks").Where("ID = ?", id).Find(&user).Error; err != nil {
		return nil, err
	}
	return &user, nil
}
...
func (repository userRepository) GetByEmail(email string) (*models.User, error) {
	var user models.User
	if err := repository.DB.Preload("Tasks").Where("email = ?", email).Find(&user).Error; err != nil {
		return nil, err
	}
	return &user, nil
}
...
func (repository userRepository) UpdateByID(user *models.User) (*models.User, error) {
	if err := repository.DB.Preload("Tasks").Where("ID = ?", user.ID).Updates(user).Error; err != nil {
		return nil, err
	}
	return user, nil
}
...
func (repository userRepository) DeleteByID(user *models.User) (*models.User, error) {
	if err := repository.DB.Preload("Tasks").Where("ID = ?", user.ID).Delete(user, user.ID).Error; err != nil {
		return nil, err
	}
	return user, nil
}
